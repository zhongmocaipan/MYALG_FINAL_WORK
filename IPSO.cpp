...
// 改进粒子群优化算法
vector<int> improvedParticleSwarmOptimization(const vector<int>& demand, int supply, int numParticles, int maxIterations) {
	...
	double w0 = 0.5, c1 = 1.0, c2 = 1.0;
	double T0 = 1.0;

	for (int iter = 0; iter < maxIterations; ++iter) {
		double w = w0 * exp(-abs(1.0 - static_cast<double>(iter) / maxIterations));
		double T = T0 * exp(-abs(1.0 - static_cast<double>(iter) / maxIterations));
		...
		for (auto& particle : particles) {
			for (int i = 0; i < numDimensions; ++i) {
				...
				particle.position[i] += static_cast<int>(particle.velocity[i] * T);
...
int main() {
	...
	vector<int> bestSolution = improvedParticleSwarmOptimization(demand, supply, numParticles, maxIterations);

	writeCSV("output_ipso.csv", bestSolution);

	cout << "改进粒子群优化结果已写入output_ipso.csv" << endl;
	return 0;
}
